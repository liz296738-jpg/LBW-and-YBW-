...
This case is intentionally labelled a *candidate* until its convergence,
supersonic-flow scope, discrete energy closure, and gamma sensitivity have been
reviewed.  The prescribed signed energy history is derived from the public NASA
Wind-US reference solution; public experiment profiles remain independent
benchmark observations and are not used to tune this run.
"""
...
def _build_case(num_cells: int, gamma: float | None) -> dict[str, Any]:
    ...
    energy_check = conservation_diagnostic(x_face, mass_flow)
    ...
    *,
    ...
    summary = {
        ...
        "flow_scope": {
            ...
            "exit_temperature_K": float(primitive.T[-1]),
            "exit_pressure_Pa": float(primitive.p[-1]),
        ...
        "claim_boundary": [
            "This is a reduced-order thermal/flow candidate driven by NASA_REFERENCE_SOLUTION_DERIVED net energy.",
            "It is not finite-rate chemistry validation and does not validate species profiles.",
            "Experimental exit profiles are not used to tune the source and remain independent benchmark context.",
