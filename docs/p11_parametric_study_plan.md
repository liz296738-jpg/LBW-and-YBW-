...
The source ledger `cases/studies/data/p11_2b_heat_release_model_source.json` contains:
...
A full regression checkpoint at commit `72d953f42e3f1ab578701db2ec9441a8c5b2be69` passed GitHub Actions run `34884490236` with `846 passed in 43.51 s`. Subsequent evidence-gate refinements must continue to pass the same full suite before acceptance.
...
The current ledger declares no formal candidate because neither complete path is available from SRC07-SRC09 in the accessible records. The machine-readable readiness state is therefore correctly blocked rather than silently filled with assumptions.
